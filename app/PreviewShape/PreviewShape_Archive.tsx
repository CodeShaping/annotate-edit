...
export type PreviewShapeArchive = TLBaseShape<
	'response',
	{
		html: string
		w: number
...
export class PreviewShapeArchiveUtil extends BaseBoxShapeUtil<PreviewShapeArchive> {
	static override type = 'response' as const

	getDefaultProps(): PreviewShapeArchive['props'] {
		return {
			html: '',
			w: (960 * 2) / 3,
			h: (540 * 2) / 3,
	...
	override component(shape: PreviewShapeArchive) {
	...
	// override toSvg(shape: PreviewShapeArchive, _ctx: SvgExportContext): SVGElement | Promise<SVGElement> {
	...
	indicator(shape: PreviewShapeArchive) {
